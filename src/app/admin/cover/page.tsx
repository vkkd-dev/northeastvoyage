...
const CoverPage = () => {
  ...
  useEffect(() => {
    const fetchImageUrl = async () => {
      const url = await getImageUrl("cover/currentImage"); // Use a constant path for the current image
      setImageUrl(url);
    };
  ...
  const handleUpload = async () => {
    ...
    const currentImageRef = ref(storage, "cover/currentImage");

    try {
      await uploadBytes(currentImageRef, selectedFile);

      const newImageUrl = await getImageUrl("cover/currentImage");
